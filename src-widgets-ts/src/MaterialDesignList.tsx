...
import { MAX_DYNAMIC_ITEMS, squarePreview, RenderProps, VisWidget, accessibleText, boundedCount, createInfo, designStyle, indexedFields, designStyleClasses, iconField, itemCount, m3Switch, parseActionValue, safeWidgetUrl, setStateValue, sizeCss, stateValue, sanitizeHtml } from './widgetUtils';
...
export default class MaterialDesignList extends VisWidget {
    ...
    static getWidgetInfo(): RxWidgetInfo { return { ...createInfo('tplVis2-materialdesign-List', 'List', attrs, ['listHeader', 'listItemLayout', 'color', 'font', 'fontSize']), visPrev: squarePreview('F0279'), visDefaultStyle: { width: 400, height: 270 } }; }
    ...
    renderWidgetBody(props: RenderProps): React.JSX.Element {
        ...
        const dividerStyle = s(data.listItemDividerStyle, 'standard'); return <React.Fragment key={index}>{row.header ? <li className={`mdc-list-group__subheader ${hSz.className}`} dangerouslySetInnerHTML={{ __html: sanitizeHtml(row.header) }} style={{ color: m3(data.colorListItemHeaders, 'var(--md-sys-color-primary)'), fontSize: hSz.fontSize, fontFamily: s(data.listItemHeaderFont) }} /> : null}<li className={`mdc-list-item${readonly ? ' mdc-list-item--disabled' : ''}`} aria-label={interactive ? accessibleText(row.text, "List item action") : undefined} data-oid={row.objectId} onClick={interactive ? () => this.activate(data, undefined, current, row) : undefined} onKeyDown={interactive ? event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); this.activate(data, undefined, current, row); } } : undefined} role={interactive ? "button" : undefined} tabIndex={interactive ? 0 : undefined} style={{ alignItems: 'center', background: active ? s(data.listItemBackgroundActive) : s(data.listItemBackground), cursor: readonly || type === 'text' ? 'default' : 'pointer', height: n(data.listItemHeight) || undefined, textAlign: s(data.listItemAlignment, 'left') as React.CSSProperties['textAlign'] }}>{image ? <span className="mdc-list-item__graphic materialdesign-icon-image" style={{ color: iconColor, fontSize: n(data.listImageHeight, 24), marginRight: n(data.distanceBetweenTextAndImage) || undefined }}>{renderIcon(image, iconColor, n(data.listImageHeight, 24), !!color)}</span> : null}<span className="mdc-list-item__text"><span className={`mdc-list-item__primary-text ${pSz.className}`} dangerouslySetInnerHTML={{ __html: sanitizeHtml(row.text) }} style={{ color: m3(data.colorListItemText, 'var(--md-sys-color-on-surface)'), fontFamily: s(data.listItemFont), fontSize: pSz.fontSize }} />{row.subText ? <span className={`mdc-list-item__secondary-text ${sSz.className}`} dangerouslySetInnerHTML={{ __html: sanitizeHtml(row.subText) }} style={{ color: m3(data.colorListItemTextSecondary, 'var(--md-sys-color-on-surface-variant)'), fontFamily: s(data.listItemSubFont), fontSize: sSz.fontSize }} /> : null}</span>{controlFirst ? [controlNode, rightNode] : [rightNode, controlNode]}</li>{row.divider ? <li className={`mdc-list-divider${dividerStyle === 'padded' ? ' mdc-list-divider--padded' : dividerStyle === 'inset' ? ' mdc-list-divider--inset' : ''}`} style={{ borderBottom: `1px solid ${m3f(data.colorListItemDivider, 'var(--md-sys-color-outline-variant)', 'rgba(0,0,0,.12)')}`, listStyle: 'none', margin: dividerStyle === 'padded' ? '0 16px' : dividerStyle === 'inset' ? '0 0 0 72px' : undefined }} /> : null}</React.Fragment>; })}</ul>;
