...
export default class MaterialDesignList extends VisWidget {
    ...
    renderWidgetBody(props: RenderProps): React.JSX.Element {
        ...
        const list = s(data.listLayout, 'standard') === 'standard' ? content : <div className={`materialdesign-list-card${s(data.listLayout) === 'cardOutlined' ? ' materialdesign-list-card--outlined' : ''}`}>{content}</div>; return <div className={`materialdesign-widget materialdesign-list${isM3 ? ` ${designStyleClasses(data, this.isDarkTheme())}` : ''}`} style={{ ...vars, ['--materialdesign-list-item-height' as string]: n(data.listItemHeight) ? `${n(data.listItemHeight)}px` : undefined, background: s(data.listBackground) || undefined, display: 'flex', flexDirection: 'column', height: '100%', width: '100%' }}><style>{listCss}</style>{header}{list}</div>;
    }
}
