...
import { MAX_DYNAMIC_ITEMS, squarePreview, RenderProps, VisWidget, boundedCount, createInfo, designStyle, designStyleClasses, iconField, setStateValue, sizeCss, stateValue, sanitizeHtml, boolValue as b, numberValue as n, textValue as s } from './widgetUtils';
...
const attrs: RxWidgetInfo['visAttrs'] = [
    { name: 'common', fields: [{ name: 'oid', label: 'oid', type: 'id' }, { name: 'showMaxAlerts', label: 'showMaxAlerts', type: 'number', default: 3 }, { name: 'minScreenResolution', label: 'minScreenResolution', type: 'number' }] },
...
export default class MaterialDesignAlerts extends VisWidget {
    // Which close button is held down — `closeIconPressColor` needs the pressed state.
    private pressedIndex = -1;
    static getWidgetInfo(): RxWidgetInfo { return { ...createInfo('tplVis2-materialdesign-Alerts', 'Alerts', attrs, ['alertLayout']), visPrev: squarePreview('F0026'), visDefaultStyle: { width: 150, height: 38 } }; }
    ...
    renderWidgetBody(props: RenderProps): React.JSX.Element {
        ...
        const hideBelow = n(data.minScreenResolution);
        const scopeClass = `mdw-alerts-${String(this.props.id || 'x').replace(/[^\w-]/g, '')}`;
        const outlined = s(data.alertLayouts) === 'outlined';
        return <div className={`materialdesign-widget materialdesign-alerts ${scopeClass}${isM3 ? ` ${designStyleClasses(data, this.isDarkTheme())}` : ''}`} style={{ height: '100%', overflow: 'visible', pointerEvents: 'none', width: '100%' }}>{hideBelow > 0 ? <style>{`@media (max-width:${hideBelow - 1}px){.${scopeClass}{display:none!important}}`}</style> : null}<div className="materialdesign-vuetify-alerts" role="status" aria-live="polite" style={{ pointerEvents: 'auto', width: '100%' }}>{shown.map((alert, index) => <div className={`v-alert ${s(data.alertLayouts, 'normal') === 'outlined' ? 'v-alert--outlined' : ''} ${s(data.alertLayouts) === 'tile' ? 'v-alert--tile' : ''}`} key={index} style={{ alignItems: 'center', background: s(alert.backgroundColor, isM3 ? 'var(--md-sys-color-surface-container)' : ''), border: isM3 && outlined ? '1px solid var(--md-sys-color-outline-variant)' : undefined, borderRadius: s(data.alertLayouts) === 'tile' ? 0 : 4, borderBottom: border === 'bottom' ? `${stripe}px solid ${s(alert.borderColor)}` : undefined, borderLeft: border === 'left' ? `${stripe}px solid ${s(alert.borderColor)}` : undefined, borderRight: border === 'right' ? `${stripe}px solid ${s(alert.borderColor)}` : undefined, borderTop: border === 'top' ? `${stripe}px solid ${s(alert.borderColor)}` : undefined, boxShadow: elevation(data.alertElevation), display: 'flex', fontFamily: s(data.alertFontFamily, 'inherit'), fontSize: sizeCss(data.alertFontSize, 16), marginBottom: n(data.alertMarginBottom, 16), minHeight: dense ? undefined : 48, paddingBottom: padV - (border === 'bottom' ? stripe : 0), paddingLeft: padH - (border === 'left' ? stripe : 0), paddingRight: padH - (border === 'right' ? stripe : 0), paddingTop: padV - (border === 'top' ? stripe : 0) }}><span className="materialdesign-v-alerts-prepend" style={{ display: 'inline-flex', marginRight: 12 }}>{alert.icon ? renderIcon(alert.icon, s(alert.iconColor, isM3 ? 'var(--md-sys-color-on-surface-variant)' : ''), n(data.alertIconSize, 24), !!alert.iconColor) : null}</span><label className="materialdesign-v-alert-text" style={{ color: s(alert.fontColor, isM3 ? 'var(--md-sys-color-on-surface)' : ''), flex: 1 }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(s(alert.text)) }} /><button aria-label={VisWidget.t('ariaCloseAlert')} className="materialdesign-icon-button v-alert-materialdesign-icon-button" onClick={() => close(index)} onPointerCancel={() => { this.pressedIndex = -1; this.forceUpdate(); }} onPointerDown={() => { this.pressedIndex = index; this.forceUpdate(); }} onPointerLeave={() => { if (this.pressedIndex === index) { this.pressedIndex = -1; this.forceUpdate(); } }} onPointerUp={() => { this.pressedIndex = -1; this.forceUpdate(); }} style={{ background: 'transparent', border: 0, cursor: 'pointer', height: 30, width: 30 }} type="button">{renderIcon(s(data.closeIcon, 'close-circle-outline'), (this.pressedIndex === index && s(data.closeIconPressColor)) || s(data.closeIconColor, isM3 ? 'var(--md-sys-color-on-surface-variant)' : '#44739e'), 20, true)}</button></div>)}</div></div>;
    }
}
