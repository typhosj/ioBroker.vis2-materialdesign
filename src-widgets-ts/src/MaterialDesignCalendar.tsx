...
export function calendarEventLanes(slots: { start: number; finish: number }[]): { index: number; total: number }[] {
    ...
    const close = (): void => {
        ...
        cluster.forEach(item => { lanes[item.id].total = total; });
        cluster = [];
    };
    for (const item of order) {
        if (cluster.every(other => other.end <= item.start)) close();
        const taken = new Set(cluster.filter(other => other.end > item.start).map(other => lanes[other.id].index));
        let index = 0;
        while (taken.has(index)) index++;
        lanes[item.id].index = index;
        cluster.push(item);
    }
    close();
    return lanes;
...
export default class MaterialDesignCalendar extends VisWidget {
    ...
    renderWidgetBody(props: RenderProps): React.JSX.Element {
        ...
        const dayBackground = m3(d.calendarDayBackgroundColor, 'var(--md-sys-color-surface)', isDark ? '#303030' : '#fff');
        ...
        const move = (amount: number): void => { this.date = new Date(this.date); if (view === 'month') { this.date.setDate(1); this.date.setMonth(this.date.getMonth() + amount); } else this.date.setDate(this.date.getDate() + amount * (view === 'week' ? 7 : 1)); this.forceUpdate(); };
        const setView = (next: string): void => { this.view = next; this.forceUpdate(); };
        // Inline styles cannot express `:hover`, so the hover color is written straight onto the node and
        // the untouched background is put back on leave.
        const hover = (color: unknown, base: string): { onMouseEnter: React.MouseEventHandler<HTMLElement>; onMouseLeave: React.MouseEventHandler<HTMLElement> } | undefined =>
            s(color) ? { onMouseEnter: event => { event.currentTarget.style.background = s(color); }, onMouseLeave: event => { event.currentTarget.style.background = base; } } : undefined;
        // `raised` and `unelevated` are filled buttons, `raised` additionally carries the elevation shadow.
        const controlLayout = s(d.controlButtonLayout, 'text');
        const controlFilled = controlLayout === 'raised' || controlLayout === 'unelevated';
        const controlBackground = s(d.controlButtonColor, controlFilled ? (isM3 ? 'var(--md-sys-color-secondary-container)' : isDark ? '#424242' : '#f5f5f5') : 'transparent');
        const controlStyle: React.CSSProperties = { background: controlBackground, border: controlLayout === 'outlined' ? `1px solid ${s(d.controlIconColor) || (isM3 ? 'var(--md-sys-color-outline)' : '#44739e')}` : 0, borderRadius: isM3 ? 'var(--md-sys-shape-corner-full)' : 4, boxShadow: controlLayout === 'raised' ? '0 3px 1px -2px rgba(0,0,0,.2), 0 2px 2px 0 rgba(0,0,0,.14), 0 1px 5px 0 rgba(0,0,0,.12)' : undefined, cursor: 'pointer', display: 'block', flexGrow: s(d.controlPosition, 'stretch') === 'stretch' ? 1 : undefined, height: 36, minWidth: n(d.controlMinWidth) || undefined, overflow: 'hidden', padding: '0 8px', position: 'relative' };
        const control = (name: string, symbol: string, action: () => void): React.JSX.Element => <div aria-label={VisWidget.t(`calendarControl${name}`)} className={`materialdesign-button materialdesign-vuetify-calendar-control-button${isM3 ? ' mdw-state-layer' : ''}`} key={name} onClick={() => { this.feedback(d); action(); }} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); this.feedback(d); action(); } }} role="button" style={controlStyle} tabIndex={0} {...hover(d.controlButtonRippelEffectColor, controlBackground)}><div className="materialdesign-button-body" style={{ alignItems: 'center', color: controlIconColor, display: 'flex', height: '100%', justifyContent: 'center' }}><span className={`materialdesign-vuetify-calendar-control-button-icon mdi mdi-${symbol}`} style={{ color: controlIconColor, fontSize: px(d.controlIconSize, 24), height: px(d.controlIconSize, 24), lineHeight: 1, width: px(d.controlIconSize, 24) }} />{b(d.controlShowLabel, true) ? <span className="materialdesign-vuetify-calendar-control-button-text" style={{ color: controlTextColor, fontFamily: s(d.controlTextFont, 'inherit'), fontSize: px(d.controlTextSize, 12), marginLeft: 4 }}>{VisWidget.t(`calendarControl${name}`)}</span> : null}</div></div>;
        const timeFormat = s(d.calendarTimeFormat, 'locale');
        // A short event only has room for one clipped line, so the full text goes on the element
        // itself — that is the desktop tooltip and what a screen reader announces.
        const eventTitle = (event: Event): string => `${calendarEventHasTime(event.start) ? `${formatCalendarTime(eventMinutes(event.start), timeFormat, timeLocale)} ` : ''}${s(event.name)}`;
        ...
        const stackEvents = s(d.calendarEventOverlapMode, 'column') === 'stack';
        ...
        const timeGrid = <div className="v-calendar v-calendar-daily theme--light v-calendar-events" style={{ display: 'flex', flex: 1, flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}><div className="v-calendar-daily__head" style={{ display: 'grid', flex: '0 0 auto', gridTemplateColumns: timeGridColumns, overflow: 'hidden', scrollbarGutter: 'stable' }}>
            ...
            {gridDays.flatMap(({ iso }, dayIndex) => {
                ...
                return placed.map(({ event, slot }, eventIndex) => {
                    ...
                    // The lane's stacking order and the minimum box height go through custom properties:
                    // the `:hover` rule that opens the full text cannot beat an inline style, and this
                    // widget set's stylesheet carries no `!important`.
                    return <div className="v-event-column" key={`event-${iso}-${eventIndex}`} style={{ ['--mdw-event-lane' as string]: 1 + index, gridColumn: dayIndex + 2, gridRow: `1 / span ${slotCount}`, minHeight: 0, pointerEvents: 'none', position: 'relative' }}><div className="v-event" title={eventTitle(event)} style={{ ['--mdw-event-left' as string]: stackEvents ? `${index * 12 + 1}px` : `calc(${index * 100 / total}% + 1px)`, ['--mdw-event-min-height' as string]: n(d.calendarEventHeight) ? `${n(d.calendarEventHeight)}px` : undefined, ['--mdw-event-width' as string]: width, backgroundColor: s(event.color, eventBackground), borderRadius: eventRadius, boxSizing: 'border-box', color: eventText(event), fontFamily: s(d.calendarEventFont, 'inherit'), fontSize: px(d.calendarEventFontSize, 12), height: `calc(${height}% - 2px)`, pointerEvents: 'auto', position: 'absolute', top: `calc(${top}% + 1px)` }}>{calendarEventHasTime(event.start) ? <div className="mdw-event-time">{formatCalendarTime(startMinute, timeFormat, timeLocale)}</div> : null}<div className="mdw-event-name">{s(event.name)}</div></div></div>;
                });
            })}
