...
export default class MaterialDesignCalendar extends VisWidget {
    ...
    renderWidgetBody(props: RenderProps): React.JSX.Element {
        ...
        const timeLocale = typeof window !== 'undefined' ? ((window as unknown as { vis?: { language?: string }; systemLang?: string }).vis?.language || (window as unknown as { systemLang?: string }).systemLang || locale) : locale;
        ...
        const timeGrid = <div className="v-calendar v-calendar-daily theme--light v-calendar-events" style={{ display: 'flex', flex: 1, flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}><div className="v-calendar-daily__head" style={{ display: 'grid', flex: '0 0 auto', gridTemplateColumns: timeGridColumns }}>
            <div className="v-calendar-daily__intervals-head" style={{ background: s(d.calendarTimeAxisHeaderBackgroundColor, 'transparent'), borderRight: `1px solid ${s(d.calendarBorderColor, '#e0e0e0')}` }} />
            {gridDays.map(({ day, iso }) => { const isToday = day.getTime() === today.getTime(); const next = view === 'week' ? s(d.calendarDayButtonWeekViewGoTo, 'day') : s(d.calendarDayButtonDayViewGoTo, 'week'); const hf = view === 'week' ? s(d.calendarWeekViewHeaderFormat) : s(d.calendarDayViewHeaderFormat); const df = view === 'week' ? s(d.calendarWeekViewDayFormat) : s(d.calendarDayViewDayFormat); const header = hf ? formatMoment(day, hf, locale) : new Intl.DateTimeFormat(locale, { weekday: 'long' }).format(day); const label = df ? formatMoment(day, df, locale) : day.getDate(); return <div className={`v-calendar-daily_head-day ${isToday ? 'v-present' : 'v-past'}`} key={`header-${iso}`} style={{ background: s(d.calendarHeaderBackground, 'transparent'), borderBottom: `1px solid ${s(d.calendarBorderColor, '#e0e0e0')}`, borderRight: `1px solid ${s(d.calendarBorderColor, '#e0e0e0')}`, borderTop: `1px solid ${s(d.calendarBorderColor, '#e0e0e0')}`, minWidth: 0, paddingBottom: 1, width: 'auto' }}><div className="v-calendar-daily_head-weekday" style={{ color: s(d.calendarDayLabelPreviousFontColor, isDark ? '#aaa' : 'rgba(0,0,0,.38)'), fontFamily: s(d.calendarDayLabelFontFamily, 'inherit'), fontSize: px(d.calendarDayLabelFontSize, 13), overflow: 'hidden', padding: 4, textAlign: 'center', textTransform: 'uppercase' }}>{header}</div><div className="v-calendar-daily_head-day-label" style={{ overflow: 'hidden', paddingBottom: 3, textAlign: 'center' }}><button className="v-btn v-btn--depressed v-btn--fab v-btn--round theme--light v-size--default transparent" type="button" onClick={() => setView(next)} style={{ alignItems: 'center', background: isToday ? s(d.calendarDayButtonTodayColor, '#44739e') : s(d.calendarDayButtonColor, 'transparent'), border: 0, borderRadius: '50%', color: isToday ? s(d.calendarDayButtonTodayFontColor, '#fff') : s(d.calendarDayButtonFontColor, isDark ? '#fff' : '#000'), cursor: 'pointer', display: 'inline-flex', fontFamily: isToday ? s(d.calendarDayButtonTodayFontFamily, 'inherit') : s(d.calendarDayButtonFontFamily, 'inherit'), fontSize: px(isToday ? d.calendarDayButtonTodayFontSize : d.calendarDayButtonFontSize, 14), height: 56, justifyContent: 'center', minWidth: 0, padding: 0, width: 56 }}><span className="v-btn__content">{label}</span></button></div></div>; })}
        </div><div className="v-calendar-daily__scroll-area" style={{ display: 'grid', flex: 1, gridTemplateColumns: timeGridColumns, gridTemplateRows: `repeat(${slotCount}, ${slotHeight}px)`, minHeight: 0, overflow: 'auto' }}>
            {Array.from({ length: slotCount }, (_, slot) => {
                const minute = firstMinute + slot * intervalMinutes;
                const showLabel = slot < slotCount - 1 && (b(d.calendarTimeAxisShortIntervals, true) || minute % 60 === 0);
                return <React.Fragment key={`slot-${minute}`}><div style={{ background: s(d.calendarTimeAxisBackgroundColor, 'transparent'), boxSizing: 'border-box', gridColumn: 1, gridRow: slot + 1, paddingTop: slotHeight }}>{showLabel ? <div className="v-calendar-daily__interval-text" style={{ boxSizing: 'border-box', color: s(d.calendarTimeAxisFontColor, isDark ? '#fff' : '#000'), display: 'block', fontFamily: s(d.calendarTimeAxisFont, 'inherit'), fontSize: px(d.calendarTimeAxisFontSize, 12), paddingRight: 4, position: 'relative', textAlign: 'right', top: -6, width: timeAxisWidth - 8 }}>{formatCalendarTime(minute + Math.floor(intervalMinutes / 2), timeFormat, timeLocale)}</div> : null}</div>{gridDays.map(({ iso }, dayIndex) => <div className="v-calendar-daily__day-interval" key={`${iso}-${minute}`} style={{ background: s(d.calendarDayBackgroundColor, 'transparent'), borderLeft: dayIndex === 0 ? 0 : undefined, borderRight: `1px solid ${s(d.calendarBorderColor, '#e0e0e0')}`, borderTop: `1px solid ${s(d.calendarBorderColor, '#e0e0e0')}`, gridColumn: dayIndex + 2, gridRow: slot + 1 }} />)}</React.Fragment>;
            })}
